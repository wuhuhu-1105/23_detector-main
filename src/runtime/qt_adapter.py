...
def bgr_to_qimage(frame_bgr: np.ndarray) -> QImage:
    ...
    return QImage(frame_rgb.data, w, h, frame_rgb.strides[0], QImage.Format.Format_RGB888).copy()
...
def render_to_qimage(
    ...
    *,
    ...
    frame = frame_bgr if no_overlay else draw_detections(frame_bgr, detections)
    return bgr_to_qimage(frame)


def output_to_status(output: FrameOutput) -> StatusDTO:
    ...
    target_ratio = None
    if output.metrics:
        target_ratio = output.metrics.get("target_ratio")
    return StatusDTO(
...
def frame_output_to_view(output: FrameOutput, *, no_overlay: bool = False) -> Tuple[QImage, StatusDTO]:
    status = output_to_status(output)
