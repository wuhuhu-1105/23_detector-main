...
class AppConfig:
    ...
    people_detector: DetectorConfig = field(
        default_factory=lambda: DetectorConfig(
            model_path=r"D:\23_detector-main\best\yolo11n.pt",
            conf=0.25,
            iou=0.45,
        )
    )
    sampling_close_detector: DetectorConfig = field(
        default_factory=lambda: DetectorConfig(
            model_path=r"D:\23_detector-main\best\2_sampling_close\best.pt",
            conf=0.25,
            iou=0.45,
        )
    )
    ...
    blocking_detector: DetectorConfig = field(
        default_factory=lambda: DetectorConfig(
            model_path=r"D:\23_detector-main\best\1_blocking\best.pt",
            conf=0.25,
            iou=0.45,
        )
    )
