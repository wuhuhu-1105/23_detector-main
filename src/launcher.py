...
class LauncherWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        outputs_root = ReportService.default_output_root()
    ...
    def _prepare_export_dirs(self) -> Tuple[str, str, str]:
        outputs_root = ReportService.default_output_root()
        reports_dir = ReportService.next_reports_dir(outputs_root)
        report_dir = ReportService.report_data_dir(reports_dir)
        ReportService.ensure_outdir(outputs_root)
        ReportService.ensure_outdir(reports_dir)
        ReportService.ensure_outdir(report_dir)
    ...
    def _update_confirm_estimate(self) -> None:
        ...
        total_frames = ReportService.get_total_frames(self._selected_video)
        if total_frames <= 0:
            self._export_confirm.update_estimate("--")
            return
    ...
    def _save_settings(self) -> None:
        ...
        save_settings(self._settings, ReportService.default_output_root())
        self._apply_settings()
        self._go_home()
