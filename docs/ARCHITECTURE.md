...
## Services isolation (collaboration boundary)
- realtime/report must not import each other (enforced by `tools/check_service_imports.py` and CI).
- Shared contracts/types must live under `src/core/contracts`.
- Other shared infra may live under `src/core/` (e.g., errors, logging_spec, settings_schema), but must not depend on UI or services.
- Contracts v0.1 freeze: only add optional fields; breaking change requires version bump + dual review.
- UI/CLI must not import `services/*_impl/*`; only depend on Service interfaces. Compat shell imports are allowed temporarily but will be removed.
- Run locally: `python tools/check_service_imports.py`

## Logging + observability (run_id)
`run_start` is a single key=value line for each run, example:
```
run_start run_id=... contracts_version=0.1 service=report service_version=0.1 source=... device=cpu device_mode=auto
```
`run_id` propagates to `effective_config`, `progress_event`, `RealtimeEvent`, and `outputs/export.log`.
First step in debugging: grep `run_id` or `run_start`.

## Local smoke (guard + A/B)
See `tools/smoke_local.md` for generating a 10s sample and running A/B.
...
```
Notes:
- Dynamic-skip UI mode does not use `VideoSource`/`iter_frame_outputs`; it reads via `cv2.VideoCapture` in `services.realtime_impl.worker.VideoWorker` (compat via `ui_qt.worker`) and runs `PipelineRunner.process_frame`.

## Thread Model
- UI main thread: owns Qt widgets and QTimer. Converts BGR -> QImage -> QPixmap in `MainWindow.render_latest`.
- Worker/QThread: `services.realtime_impl.worker.VideoWorker` performs capture + inference and emits `frame_ready(frame_bgr, FrameOutput)` plus `error(str)` (compat via `ui_qt.worker`).
...
- Headless runner: `services.realtime_impl.runner.run_headless` runs the same pipeline without UI (compat via `runtime.runner`), writing jsonl + summary files.
...
- Calls/Depends on: `runtime.cli`, `runtime.config_overrides`, `services.realtime_impl.runner` (via compat), `ui_qt.main_window`, `services.realtime_impl.worker` (via compat).
...
- Calls/Depends on: `services.report_service`, `services.report_impl.export_core`, `report.builder`, `report.writer_json`, `report.writer_docx`, `report.video_export`.
Notes:
...
- Responsibility: compatibility shell for headless runner.
- Key classes/functions: re-exports from `services.realtime_impl.runner`.
- Inputs/Outputs: same as `services.realtime_impl.runner`.
- Called by: entrypoints.
- Calls/Depends on: `services.realtime_impl.runner`.
...
#### `src/ui_qt/worker.py`
- Responsibility: compatibility shell for realtime worker.
- Key classes/functions: re-exports from `services.realtime_impl.worker`.
- Inputs/Outputs: same as `services.realtime_impl.worker`.
- Called by: `src/app_qt.py`.
- Calls/Depends on: `services.realtime_impl.worker`.
...
#### `src/report/export_core.py`
- Responsibility: compatibility shell for report export orchestration.
- Key classes/functions: re-exports from `services.report_impl.export_core`.
- Inputs/Outputs: same as `services.report_impl.export_core`.
- Called by: legacy imports only.
- Calls/Depends on: `services.report_impl.export_core`.

### services

#### `src/services/realtime_impl/runner.py`
...
- Called by: entrypoints via `runtime.runner`.
- Calls/Depends on: `runtime.pipeline`, `runtime.summary`, `io.video_writer`.

#### `src/services/realtime_impl/worker.py`
...
- Called by: `src.app_qt.py` via `RealtimeService`.
- Calls/Depends on: `runtime.pipeline`, `runtime.frame_scheduler`, `io.video_writer`.

#### `src/services/report_impl/export_core.py`
- Responsibility: report export orchestration (paths, stages, progress).
- Key classes/functions: `run_export`, `next_reports_dir`, `get_total_frames`.
- Inputs/Outputs: CLI args + report config -> report outputs.
- Called by: `ReportService.export()`.
- Calls/Depends on: `report.builder`, writers, `report.video_export`.
